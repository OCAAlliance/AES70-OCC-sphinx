...
:ref:`OcaRoot <ocaroot>` : :ref:`OcaManager <ocamanager>` : :ref:`OcaFirmwareManager <ocafirmwaremanager>`
...
.. cpp:class:: OcaFirmwareManager: OcaManager
    ...
    this class's functions needed to report firmware version numbers to
    inquiring controllers. This firmware manager offers a generic interface for
    updating OCA devices. The actual robustness of the update process is left up
    to the implementer. The interface allows for any of: - Fully transactional
    based uploads (i.e. only committing to the newly uploaded images after all
    component uploads have succeeded, and reverting back to the old images if
    any step fails) - Partly transactional based uploads (i.e. committing to a
    newly uploaded image after each individual component upload succeeds,
    ...
    **Properties**:
    ...
    .. cpp:member:: OcaList<OcaVersion> ComponentVersions
        ...
        List of the versions of the components of the device. As of version 2 of
        this class, component numbers are of datatype** OcaEnum,** rather than
        the previous **OcaUint16.**
    ...
    **Methods**:
    ...
    .. cpp:function:: OcaStatus GetComponentVersions(OcaList<OcaVersion> &componentVersions)
        ...
        - :cpp:expr:`componentVersions`: Output parameter.
    ...
    .. cpp:function:: OcaStatus BeginActiveImageUpdate(OcaComponent component)
        ...
        generic interface can be used to update any component which can be
        updated actively, i.e. where the upload tool actively pushes the
        ...
        - :cpp:expr:`component`: Input parameter.
    ...
    .. cpp:function:: OcaStatus AddImageData(OcaUint32 id, OcaBlob imageData)
        ...
        - :cpp:expr:`imageData`: Input parameter.
    ...
    .. cpp:function:: OcaStatus VerifyImage(OcaBlob verifyData)
        ...
        - :cpp:expr:`verifyData`: Input parameter.
    ...
    .. cpp:function:: OcaStatus BeginPassiveComponentUpdate(OcaComponent component, OcaNetworkAddress serverAddress, OcaString updateFileName)
        ...
        retrieve the new software/firmware image from the server and update its
        component using this image. The actual method for retrieving the image
        (e.g. TFTP) and the underlying update technique (e.g. netflash) depend
        on the implementation and may differ between components. Just the
        ...
        - :cpp:expr:`serverAddress`: Input parameter.


        - :cpp:expr:`updateFileName`: Input parameter.
